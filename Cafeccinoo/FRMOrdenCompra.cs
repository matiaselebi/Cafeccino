...
namespace Cafeccinoo
{
    public partial class FRMOrdenCompra : Form, IObserver
    {
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                foreach (DataGridViewRow dr in dataGridView2.Rows)
                {
                    ...
                    {
                        MessageBox.Show(LanguageManager.ObtenerInstancia().ObtenerTexto("FRMOrdenCompra.Etiquetas.ProductoYaSeleccionado"));
        ...
        private void dataGridView2_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                MessageBox.Show(LanguageManager.ObtenerInstancia().ObtenerTexto("FRMOrdenCompra.Etiquetas.ProductoEliminado") + dataGridView2.Rows[e.RowIndex].Cells[2].Value.ToString());
        ...
        {
            DataTable dt = negocios.ObtenerTabla("*", "Producto", $"Activo = 1 AND CodProducto LIKE '{textBox1.Text}%'");
            dataGridView1.DataSource = dt;
        }
        ...
        private void BTNRegistrarOrden_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                foreach (DataGridViewRow dr in dataGridView2.Rows)
                {
                    orden.Items.Add(new ItemOrden(Convert.ToInt32(dr.Cells[0].Value.ToString()), CodOrden, Convert.ToDouble(dr.Cells[3].Value), Convert.ToInt32(dr.Cells[4].Value)));
                }
        ...
        {
            DataTable ProductosCompra = negocios.ObtenerTabla("Producto.CodProducto, Tipo, Nombre, Cotizacion, StockCompra", "Producto INNER JOIN ItemOrden ON Producto.CodProducto = ItemOrden.CodProducto", $"CodOrdenCompra = {NegociosOrdenCompra.ObtenerCodOrdenCompra()}");
