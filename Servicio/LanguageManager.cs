...
namespace Servicio
{
    public class LanguageManager : ISubject
    {
        ...
        public string ObtenerTexto(string key, DataTable dt = null)
        {
            if (Diccionario == null)
                Diccionario = new Dictionary<string, string>();

            string nuevoTexto = Diccionario.ContainsKey(key) ? Diccionario[key] : key;

            // Evita duplicados en la DataTable
            if (dt != null)
            {
                int contador = 1;
                string textoOriginal = nuevoTexto;

                while (dt.Columns.Contains(nuevoTexto))
                {
                    nuevoTexto = $"{textoOriginal}_{contador}";  // Agrega un sufijo incremental
                    contador++;
                }
            }
