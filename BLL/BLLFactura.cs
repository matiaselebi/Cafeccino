...
namespace BLL
{
    public class BLLFactura
    {
        ...
        public void RegistrarItems(Factura fact)
        {
            ...
            {
                Data.EjecutarComando("RegistrarItem", $"{item.CodFactura}, '{item.CodProducto}', {item.Cantidad}");
            }
        ...
        public void GenerarFactura(Factura fact, DataTable Productos)
        {
            ...
            else
            {
                ...
                foreach (DataRow dr in Productos.Rows)
                {
                    int CodProducto = dr[0] != DBNull.Value ? Convert.ToInt32(dr[0]) : 0;
                    int cantidad = dr[4] != DBNull.Value ? Convert.ToInt32(dr[4]) : 0;

                    fact.Items.Add(new Item(CodFact, CodProducto, cantidad));
                }
                RegistrarItems(fact);
